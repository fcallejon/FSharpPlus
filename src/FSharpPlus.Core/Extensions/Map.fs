...
module Map =
    ...
    let inline chooseValues ([<InlineIfLambda>]mapping) (source: Map<'Key, 'T>) = Map <| seq {
    ...
    let inline unionWith ([<InlineIfLambda>]combiner) (source1: Map<'Key, 'Value>) (source2: Map<'Key, 'Value>) =
    ...
    let inline choosei ([<InlineIfLambda>]f: 'Key -> 'T -> 'U option) (x: Map<'Key, 'T>) = Map <| seq {
