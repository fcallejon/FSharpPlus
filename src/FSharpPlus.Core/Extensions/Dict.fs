...
module Dict =
    ...
    let inline map ([<InlineIfLambda>]mapper) (source: IDictionary<'Key, 'T>) =
    ...
    let inline map2 ([<InlineIfLambda>]mapper) (source1: IDictionary<'Key, 'T1>) (source2: IDictionary<'Key, 'T2>) =
        let dct = Dictionary<'Key, 'U> ()
    ...
    let inline chooseValues ([<InlineIfLambda>]chooser) (source: IDictionary<'Key, 'T>) =
        let dct = Dictionary<'Key, 'U> ()
        for KeyValue(k, v) in source do
    ...
    let inline unionWith ([<InlineIfLambda>]combiner) (source1: IDictionary<'Key, 'Value>) (source2: IDictionary<'Key, 'Value>) =
