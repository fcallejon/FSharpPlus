...
module Seq =
    ...
    let inline bind ([<InlineIfLambda>]mapping: 'T->seq<'U>) source = Seq.collect mapping source
    ...
    let inline lift2 ([<InlineIfLambda>]f) x1 x2 = Seq.allPairs x1 x2 |> Seq.map (fun (x, y) -> f x y)
    ...
    let inline lift3 ([<InlineIfLambda>]f) x1 x2 x3 =
        Seq.allPairs x2 x3
        |> Seq.allPairs x1
    ...
    let inline chunkBy ([<InlineIfLambda>]projection: 'T -> 'Key) (source: _ seq) = seq {
