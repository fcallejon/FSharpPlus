...
module IReadOnlyDictionary =
    ...
    let inline mapValues ([<InlineIfLambda>]mapper) (source: IReadOnlyDictionary<'Key, 'T>) =
    ...
    let inline chooseValues ([<InlineIfLambda>]chooser) (source: IReadOnlyDictionary<'Key, 'T>) =
        let dct = Dictionary<'Key, 'U> ()
        for KeyValue(k, v) in source do
    ...
    ///
    /// <returns>The resulting read-only dictionary comprising the entries <c>v</c> where the chooser function returned <c>Some(x)</c>.</returns>
    let inline choose ([<InlineIfLambda>]chooser) (source: IReadOnlyDictionary<'Key, 'T>) =
        let dct = Dictionary<'Key, 'U> ()
        for KeyValue(k, v) in source do
    ...
    let inline intersectWith ([<InlineIfLambda>]combiner) (source1:IReadOnlyDictionary<'Key, 'T>) (source2:IReadOnlyDictionary<'Key, 'T>) =
        Enumerable
            .Join(
            source1,
            source2,
    ...
    let inline fold     ([<InlineIfLambda>]folder: 'State -> 'Key -> 'T -> 'State) (state: 'State) (source: IReadOnlyDictionary<'Key, 'T>) =
        let unzip source = Seq.map fst source, Seq.map snd source
        source |> toSeq |> Seq.map (|KeyValue|) |> unzip ||> Seq.fold2 folder state

    /// Folds over the bindings in the Dictionary
    let inline foldBack ([<InlineIfLambda>]folder: 'Key -> 'T -> 'State -> 'State) (source: IReadOnlyDictionary<'Key, 'T>) state =
        let unzip source = Seq.map fst source, Seq.map snd source
