...
module List =
    ...
    /// </code>
    /// </example>
    let inline apply (f: list<'T -> 'U>) (x: list<'T>) : list<'U> =
    #if FABLE_COMPILER
    ...
    let inline lift2 ([<InlineIfLambda>]f: 'T1 -> 'T2 -> 'U) (x1: list<'T1>) (x2: list<'T2>) =
    #if FABLE_COMPILER
    ...
    /// </returns>
    let inline partitionMap ([<InlineIfLambda>]mapping: 'T -> Choice<'T1, 'T2>) (source: list<'T>) =
    #if FABLE_COMPILER
    ...
    /// <returns>List with corresponding results of applying the mapping function pairwise over both input lists elements.</returns>
    ...
    let inline choosei ([<InlineIfLambda>]mapping) source =
        let mutable i = ref -1
        let fi x =
            i.Value <- i.Value + 1
            mapping i.Value x
