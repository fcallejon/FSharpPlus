...
module Choice =
    ...
    /// This is as if Choice1Of2 represents a Success value and Choice2Of2 a Failure.
    ...
    let inline map ([<InlineIfLambda>] mapping: 'T->'U) (source: Choice<'T,'T2>) = match source with Choice1Of2 v -> Choice1Of2 (mapping v) | Choice2Of2 e -> Choice2Of2 e
    ...
    let inline catch ([<InlineIfLambda>]f: 't -> _) = function Choice1Of2 v -> Choice1Of2 v | Choice2Of2 e -> f e : Choice<'v,'e>
    ...
    /// <param name="source">The source input value.</param>
    /// <returns>A result of the output type of the binder.</returns>
    let inline bindChoice2Of2 ([<InlineIfLambda>]binder: 'T2->Choice<'T,'U2>) (source: Choice<'T,'T2>) = match source with Choice1Of2 v -> Choice1Of2 v | Choice2Of2 e -> binder e
    ...
    let inline either ([<InlineIfLambda>]fChoice1Of2) ([<InlineIfLambda>]fChoice2Of2) source = match source with Choice1Of2 v -> fChoice1Of2 v | Choice2Of2 e -> fChoice2Of2 e

    /// Creates a safe version of the supplied function, which returns a Choice<'U,exn> instead of throwing exceptions.
    let inline protect ([<InlineIfLambda>]f: 'T->'U) x =
        try
            Choice1Of2 (f x)
        with e -> Choice2Of2 e
