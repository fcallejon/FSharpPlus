...
module String =
    ...
    let inline skipWhile ([<InlineIfLambda>]predicate: char -> bool) (source: string) =
        if String.IsNullOrEmpty source then
            String.Empty
        else
            let mutable i = 0
    ...
    let inline findIndex ([<InlineIfLambda>]predicate: char -> bool) (source: string) =
        let rec go index =
    ...
    let inline tryFindIndex ([<InlineIfLambda>]predicate: char -> bool) (source: string) =
        let rec go index =
    ...
    let getBytes (encoding: Encoding) (source: string) : byte [] = encoding.GetBytes source
