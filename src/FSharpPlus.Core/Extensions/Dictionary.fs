...
module Dictionary =
    ...
    let inline map ([<InlineIfLambda>]mapping) (source: Dictionary<'Key, 'T>) =
    ...
    let inline map2 ([<InlineIfLambda>]mapping) (source1: Dictionary<'Key, 'T1>) (source2: Dictionary<'Key, 'T2>) =
        let dct = Dictionary<'Key, 'U> ()
    ...
    ///
    /// <returns>The mapped Dictionary.</returns>
    let inline map3 ([<InlineIfLambda>]mapping) (source1: Dictionary<'Key, 'T1>) (source2: Dictionary<'Key, 'T2>) (source3: Dictionary<'Key, 'T3>) =
        let dct = Dictionary<'Key, 'U> ()
    ...
    let inline chooseValues ([<InlineIfLambda>]mapper) (source: IDictionary<'Key, 'T>) =
        let dct = Dictionary<'Key, 'U> ()
        for KeyValue(k, v) in source do
            match mapper v with
            | Some v -> dct.Add (k, v)
   ...
   ///Returns the intersection of two maps, preferring values from the first in case of duplicate keys.
    ...
    let inline choosei ([<InlineIfLambda>]chooser) (source: IDictionary<'Key, 'T>) =
        let dct = Dictionary<'Key, 'U> ()
        for KeyValue(k, v) in source do
