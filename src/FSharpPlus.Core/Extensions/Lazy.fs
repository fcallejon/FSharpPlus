...
module Lazy =
    ...
    let inline map ([<InlineIfLambda>]mapping: 'T -> 'U) (x: Lazy<'T>) = Lazy<_>.Create (fun () -> mapping x.Value) : Lazy<'U>
    ...
    let inline map2 ([<InlineIfLambda>]mapping: 'T->'U->'V) (x: Lazy<'T>) (y: Lazy<'U>) = Lazy<_>.Create (fun () -> mapping x.Value y.Value)
    ...
    ///
    /// <returns>The combined value.</returns>
    let inline map3 ([<InlineIfLambda>]mapping: 'T->'U->'V->'W) (x: Lazy<'T>) (y: Lazy<'U>) (z: Lazy<'V>) = Lazy<_>.Create (fun () -> mapping x.Value y.Value z.Value)
